            ...
            while (!fetch_exit)
            {
                ...
                if (frames.Count() > 0)
                {
                    ...
                    { 
                        ...
                        {
                            //depth raw data
                            Mat cv_depth = Mat.FromPixelData(frames[i].height, frames[i].width, MatType.CV_16U, frames[i].buffer.getCPtr());
                            Cv2.ImShow("Raw-Depth", cv_depth);
                            
                            cl.DeviceStreamDepthRender(frames[i], depth);
                            Mat cv_render_depth = Mat.FromPixelData(frames[i].height, frames[i].width, MatType.CV_8UC3, depth.buffer.getCPtr(), 3 * frames[i].width);
                ...
                if (frames.Count() > 0)
                {
                    for (int i = 0; i < frames.Count(); i++)
                    {
                        if (frames[i].streamID == PERCIPIO_STREAM_COLOR)
                        {
                            ...
                            Mat cv_bgr = Mat.FromPixelData(frames[i].height, frames[i].width, MatType.CV_8UC3, bgr.buffer.getCPtr(), 3 * frames[i].width);
                ...
                if (frames.Count() > 0)
                {
                    for (int i = 0; i < frames.Count(); i++)
                    {
                        if (frames[i].streamID == PERCIPIO_STREAM_IR_LEFT)
                        {
                            ...
                            Mat cv_leftIR = Mat.FromPixelData(leftIR.height, leftIR.width, MatType.CV_8UC3, leftIR.buffer.getCPtr(), 3 * leftIR.width);
                        ...
                        if (frames[i].streamID == PERCIPIO_STREAM_IR_RIGHT)
                        {
                            ...
                            Mat cv_rightIR = Mat.FromPixelData(rightIR.height, rightIR.width, MatType.CV_8UC3, rightIR.buffer.getCPtr(), 3 * rightIR.width);
                ...
                if (frames.Count() > 0)
                {
                    for (int i = 0; i < frames.Count(); i++)
                    {
                        if (frames[i].streamID == PERCIPIO_STREAM_DEPTH)
                        {
                            ...
                            Mat cv_raw_depth = Mat.FromPixelData(registration_depth.height, registration_depth.width, MatType.CV_16U, registration_depth.Ptr().getCPtr());
                            Cv2.ImShow("Raw-Depth", cv_raw_depth);
                    ...
                    Mat cv_depth = Mat.FromPixelData(registration_depth_render.height, registration_depth_render.width, MatType.CV_8UC3, registration_depth_render.Ptr().getCPtr());
                    Cv2.ImShow("Registration-Depth", cv_depth);

                    Mat cv_rsz_bgr = new Mat();
                    Mat cv_bgr = Mat.FromPixelData(undsitortion_color.height, undsitortion_color.width, MatType.CV_8UC3, undsitortion_color.buffer.getCPtr());
                ...
                if (frames.Count() > 0)
                {
                    for (int i = 0; i < frames.Count(); i++)
                    {
                        if (frames[i].streamID == PERCIPIO_STREAM_DEPTH)
                        {
                            ...
                            Mat cv_p3d = Mat.FromPixelData(frames[i].height, frames[i].width, MatType.CV_32FC3, p3d_list.getPtr().getCPtr());
                            Cv2.ImShow("PointCloud", cv_p3d);
                ...
                if (frames.Count() > 0)
                {
                    for (int i = 0; i < frames.Count(); i++)
                    {
                        if (frames[i].streamID == PERCIPIO_STREAM_DEPTH)
                        {
                            ...
                            Mat cv_depth = Mat.FromPixelData(render_depth.height, render_depth.width, MatType.CV_8UC3, render_depth.buffer.getCPtr());
                        ...
                        {
                            cl.DeviceStreamImageDecode(frames[i], color);
                            Mat cv_bgr = Mat.FromPixelData(color.height, color.width, MatType.CV_8UC3, color.buffer.getCPtr());
